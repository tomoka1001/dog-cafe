...
<section>
    <div class="mb-6 py-4 bg-white rounded">
        <div class="flex px-6 pb-4 border-b">
            <h2 class="text-xl font-bold">スタッフ一覧</h2>
            <div class="ml-auto">
                <a href="/admin/staffs/create" class="py-2 px-3 text-xs text-white font-semibold bg-indigo-500 rounded-md">スタッフ追加</a>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    <tr @class(['text-sm', 'bg-gray-50' => $loop->odd])>
                        <td class="flex px-4 py-3 items-center">
                            <p class="font-medium"> <a href="{{ route('admin.staffs.edit', ['staff' => $staff->id]) }}">{{ $staff->name }}</a>
                        </td>
                        <td>{{ $staff->updated_at }}</td>
                        <td>
                            <div class="flex">
                                <a class="mr-2" href="{{ route('admin.staffs.edit', ['staff' => $staff->id]) }}">
                                ...
                                <form action="{{ route('admin.staffs.destroy', ['staff' => $staff->id]) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button>
