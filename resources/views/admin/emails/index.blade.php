...
<section>
    <div class="mb-6 py-4 bg-white rounded">
        <div class="flex px-6 pb-4 border-b">
            <h2 class="text-xl font-bold">メール一覧</h2>
        </div>
                ...
                <tbody>
                    ...
                    <tr @class(['text-sm', 'bg-gray-50' => $loop->odd])>
                        <td class="flex px-4 py-3 items-center">
                            <p class="font-medium"> <a href={{ route('admin.emails.show', ['email' => $email->id]) }}>{{ $email->name }}</a>
                        </td>
                        <td>{{ $email->updated_at }}</td>
                        <td>
                            <div class="flex">
                                <a class="mr-2" href={{ route('admin.emails.show', ['email' => $email->id]) }}>
