...
<section>
    <div class="mb-6 py-4 bg-white rounded">
        {{-- <div class="flex px-6 pb-4 border-b">
            <h2 class="text-xl font-bold">WAN's一覧</h2>
            <div class="ml-auto">
                <a href="/admin/dogs/create" class="py-2 px-3 text-xs text-white font-semibold bg-indigo-500 rounded-md">新規投稿</a>
            </div>
        </div> --}}
        <div>
            <h2>WAN's一覧</h2>
            <a href="/admin/dogs/create">新規追加</a>
        </div>
                ...
                <tbody>
                    ...
                    <tr @class(['text-sm', 'bg-gray-50' => $loop->odd])>
                        <td class="flex px-4 py-3 items-center">
                            <img class="w-12 h-12 mr-4 object-cover rounded-md" src="{{ asset('storage/'. $dog->image) }}" alt="">
                            <p class="font-medium"> <a href="{{ route('admin.dogs.edit', ['dog' => $dog->id]) }}">{{ $dog->name }}</a></p>
                        </td>
                        <td>{{ $dog->updated_at }}</td>
                        <td>
                            <div class="flex">
                                <a class="mr-2" href="{{ route('admin.dogs.edit', ['dog' => $dog->id]) }}">
                                ...
                                <form action="{{ route('admin.dogs.destroy', ['dog' => $dog->id]) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button>
